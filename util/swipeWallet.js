...
program
  ...
  .option('-f, --fee [n]', 'Optional, fee in GRS (default 0.0001 GRS), only if amount is not provided', parseFloat)
  .parse(process.argv);
        ...
        async.eachSeries(signers, function(dummy, cb) {
          ...
          function(err) {
            ...
            if (p.builder.isFullySigned()) {
              ...
              firstWallet.broadcastToGroestlcoinNetwork(ntxid, function(txid) {
                console.log('\t #######  SENT  TXID:', txid);
                process.exit(1);
              });
            } else {
